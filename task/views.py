...
from .forms import (
    ...
    Subtaskform,
    Subtaskbyleadform,
    Allocateemployeeform,
    Allocateemployeebyleadform,
...
class TaskSearchView(View):
    def get(self, request):
        ...
        task_list_list = Task.objects.filter(
            Q(name__icontains=q) | Q(status__icontains=q)
        )
        if task_list_list:
            ...
            return render(
                request, "task/tasklist.html", {"task_list": task_list, "q": q}
            )
        else:
            ...
            try:
                ...
                task_list = paginator.page(paginator.num_pages)
            return render(
                request, "task/tasklist.html", {"task_list": task_list, "error": error}
            )
...
class TaskforleadList(ListView):
    ...
    def get_queryset(self):
        project = Project.objects.filter(
            teamlead__in=self.request.user.teamlead_set.all()
        )
...
class SearchforleadView(View):
    def get(self, request):
        ...
        task_list_list = Task.objects.filter(
            Q(name__icontains=q) | Q(status__icontains=q), Q(project__in=project)
        )
        if task_list_list:
            page = request.GET.get("page", 1)
            paginator = Paginator(task_list_list, 4)
            ...
            task_list_list = Task.objects.filter(project__in=project).order_by(
                "project_id"
            )
            page = request.GET.get("page", 1)
...
class SearchforemployeeView(View):
    def get(self, request):
        q = request.GET.get("q", None)
        projectlist = []
        for p in Task.objects.filter(
            taskallocation__in=self.request.user.taskallocation_set.all()
        ):
            projectlist.append(p.project.id)
        task_list_list = Task.objects.filter(
            Q(name__icontains=q) | Q(status__icontains=q), Q(project__in=projectlist)
        )
        if task_list_list:
            page = request.GET.get("page", 1)
            paginator = Paginator(task_list_list, 4)
...
class TaskdetailView(View):
    ...
    def get(self, request, id):
        ...
        return render(
            ...
            {"task": task, "dev": dev, "error": "error", "qa": qa, "ui": ui},
        )
...
class TaskView(View):
    ...
    def post(self, request, **kwargs):
        ...
        if request.POST.get("deadline") and str(timezone.now().date()) <= str(
...
class TaskbyleadView(View):
    ...
    def get(self, request):
        ...
        return render(
            request, "task/taskbylead.html", {"form": form, "project": project}
        )
    ...
    def post(self, request):
        ...
        return render(
            request,
            "task/taskbylead.html",
            {"form": form, "errors": errors, "project": project},
        )
        ...
        if request.POST.get("deadline") and str(timezone.now().date()) <= str(
            ...
            if request.POST.get("task"):
                Task.objects.create(
                    ...
                    description=description,
                    deadline=request.POST.get("deadline"),
                    parent_id=request.POST.get("task"),
                    status=request.POST.get("status"),
                )
                return redirect("tasklist")
            else:
                return render(
                    request,
                    "task/subtask.html",
                    {"form": form, "error": error, "project": project},
                )
        else:
            return render(
                request,
...
class SubtaskbyleadView(View):
    ...
    def get(self, request):
        ...
        return render(
            request, "task/subtaskbylead.html", {"form": form, "project": project}
        )
        ...
        if request.POST.get("deadline") and str(timezone.now().date()) <= str(
            ...
            if request.POST.get("task"):
                Task.objects.create(
                    ...
                    description=description,
                    deadline=request.POST.get("deadline"),
                    parent_id=int(request.POST.get("task")),
                    status=request.POST.get("status"),
                )
                return redirect("tasklist")
            else:
...
class SubtaskbydeveloperView(View):
    ...
    def get(self, request):
        ...
        projects = ProjectAllocation.objects.filter(
        ...
        if request.POST.get("deadline") and str(timezone.now().date()) <= str(
            ...
            if request.POST.get("task"):
                ...
                return render(
                    request,
                    "task/subtaskbydeveloper.html",
                    {"form": form, "task": task, "error": error},
                )
        else:
            return render(
                request,
                "task/subtaskbydeveloper.html",
                {"form": form, "task": task, "errors": errors},
            )
...
class AllocateemployeeView(View):
    ...
    def get(self, request):
        ...
        return render(
            request, "task/allocateemployee.html", {"form": form, "project": project}
        )
    ...
    def post(self, request, **kwargs):
        ...
        errors = (
            "Please assign other employee or make percentage value less than or equal to "
            + percent
            + "."
        )
        if request.POST.get("percentage"):
            ...
            if form.is_valid() and updated_value < 101:
                User.objects.filter(id=int(request.POST.get("employee"))).update(
                ...
                TaskAllocation.objects.create(
                    ...
                    allocation=int(request.POST.get("percentage")),
                )
                return redirect("tasklist")
        return render(
            request,
...
class AllocateemployeebyleadView(View):
    ...
    def get(self, request):
        ...
        return render(
            request,
            "task/allocateemployeebylead.html",
    ...
    def post(self, request, **kwargs):
        ...
        if updated_value > 101:
            ...
            User.objects.filter(id=int(request.POST.get("user"))).update(
                percentage=updated_value
            )
...
def validate_task(request):
    ...
    tasks = serializers.serialize(
        "json",
        Task.objects.filter(project_id=int(project), parent=None),
        fields=("name"),
    )
    employee = (
        ProjectAllocation.objects.filter(project_id=int(project))
        .values("user__username", "user", "user__percentage")
...
class SortView(View):
    def get(self, request):
        ...
        return render(
            request,
            "task/tasklist.html",
            {"task_list": task_list, "name": name, "q": q},
        )
