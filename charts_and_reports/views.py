...
class AllocationchartView(View):
    ...
    def get(self, request):
        data = {
            "projects": chartquery().get("projects"),
            "allocation": chartquery().get("allocation"),
        }
...
class EmployeegraphView(View):
    ...
    def get(self, request):
        ...
        data = {
            "projects": employeechart(request.GET.get("user", None)).get("projects"),
            "allocation": employeechart(request.GET.get("user", None)).get(
                "allocation"
            ),
        }
        return JsonResponse(data, safe=False)
...
class ReportView(View):
    ...
    def get(self, request):
        return render(
            request,
            "registration/emplyoeereport.html",
    ...
    def post(self, request):
        ...
        if self.request.user.is_staff or self.request.user.is_superuser:
            ...
            if not year:
                ...
                return render(
                    request,
                    "registration/emplyoeereport.html",
                    {"employee": User.objects.all(), "error": error},
                )
        ...
        if "download" in request.POST:
            ...
            data = [
                ...
                for element1, element2, element3, element4 in zip(
                    month_list, project_list, allocation_list, role_list
                )
            ]
            if data:
                return render(
